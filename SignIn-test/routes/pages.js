...
router.get("/reviews", (req, res) => {
  ...
  res.render("reviews", {
    user_data: req.session.user_data[0],
    storeclicked: req.session.storeclicked.store,
  });
});
...
router.get("/discounts", (req, res) => {
  ...
  db.query(
    "SELECT discount.*, users.*, product.* FROM discount INNER JOIN users ON discount.entry_by = users.userID INNER JOIN product ON product.counter=discount.counter WHERE storeID = ? AND discount.entry_date = (SELECT MAX(entry_date) FROM discount WHERE discount.counter = product.counter) GROUP BY discount.counter ORDER BY discount.entry_date DESC",
    [req.session.storeclicked.store.storeID],
    (error, results) => {
      if (error) {
...
router.get("/products", (req, res) => {
  db.query(
    "SELECT product.* , stores.store_name FROM product JOIN stores WHERE stores.storeID = product.storeID",
    (error, results) => {
      if (error) {
        res.status(500).send(error);
      }
      ...
      db.query("SELECT * FROM discount", (err, rest) => {
        if (err) {
          ...
          res.json([
            results,
            rest,
            req.session.user_data[0],
            req.session.storeclicked,
          ]);
        }
      });
    }
...
router.get("/user_history", (req, res) => {
  ...
  db.query(
    "SELECT * FROM interaction INNER JOIN product ON product.counter = interaction.counter WHERE userID = ? ORDER BY interaction.timestamp DESC",
    [req.session.user_data[0].userID],
    (error, results) => {
      if (error) {
        res.status(500).send(error);
      }
      ...
      db.query(
        "SELECT * FROM discount INNER JOIN product ON product.counter = discount.counter WHERE entry_by = ? ORDER BY discount.entry_date DESC",
        [req.session.user_data[0].userID],
        (err, rest) => {
          if (err) {
            res.status(500).send(err);
...
router.get("/userslead", (req, res) => {
  // the following query will give me all the interactions of the user loged in, for the products with counter same as the ones in the interaction table
  db.query(
    "SELECT * FROM users WHERE total_score > 0 ORDER BY users.total_score DESC",
    (error, results) => {
      //
      if (error) {
        res.status(500).send(error);
